...
bool canCompleteTasks(int maxWork, int tasks[], int numTasks, int days) {
...
int findMinDailyWork(int tasks[], int numTasks, int maxDays) {
    int minWork = 1;
    int maxWork = INT_MAX;
    ...
    while (minWork <= maxWork) {
        ...
        if (canCompleteTasks(midWork, tasks, numTasks, maxDays)) {
...
int main() {
    ...
    int* tasks = new int[numTasks]; // Dynamic array to hold task work values
    ...
    int result = findMinDailyWork(tasks, numTasks, maxDays);
